...
cv::Mat NDArrayConverter::toMat(PyObject *o) {
    cv::Mat m;
    NPE_PY_ARRAY_OBJECT* o_as_array_ptr =
            reinterpret_cast<NPE_PY_ARRAY_OBJECT*>(o);
    ...
    int typenum = PyArray_TYPE(o_as_array_ptr);
    ...
    int ndims = PyArray_NDIM(o_as_array_ptr);
    ...
    const npy_intp* _sizes = PyArray_DIMS(o_as_array_ptr);
    const npy_intp* _strides = PyArray_STRIDES(o_as_array_ptr);
    ...
    m = Mat(ndims, size, type, PyArray_DATA(o_as_array_ptr), step);
